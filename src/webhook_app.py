...
async def lifespan(app: FastAPI):
    ...
    print(f"Webhook установлен: {config.telegram.webhook.url}")
    yield
    await bot.delete_webhook()
    await bot.session.close()
    print("Webhook удален, бот отключен.")
...
async def webhook_handler(update: dict):
    try:
        ...
        if config.debug:
            print(f"Got update | update_id={update.update_id} | from={from_username}")
